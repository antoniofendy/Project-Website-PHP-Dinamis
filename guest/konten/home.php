<?php
	...
	if(!defined("INDEX")){
		...
		$kueri = "SELECT * FROM artikel ORDER BY id_artikel DESC LIMIT $posisi,$batas";
...
<div id="blok_artikel">
	...
	<p>
		...
		<?= $isi_tampil; ?> ... <a href="?tampil=artikel_detail&id=<?= $data['id_artikel']; ?>" target="blank" class = "link_lengkap">Selengkapnya</a>
...
<div class="paging">
	...
	<?php
		...
		$semua = mysqli_query($koneksi, "SELECT * FROM artikel") or die(mysqli_error($koneksi));
		$jml_artikel = mysqli_num_rows($semua);

		$hal_sblm = $hal-1;
		$hal_ssdh = $hal+1;
		$jml_hal = ceil($jml_artikel/5);
